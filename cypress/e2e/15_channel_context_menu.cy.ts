describe('Channel Context Menu', () => {
  ...
  it('opens the main context menu and displays create options, then shows the create-category form', () => {
    ...
    cy.get('.context-menu .menu-item').contains('Create Channel').should('be.visible');
    cy.get('.context-menu .menu-item').contains('Create Category').should('be.visible');
    cy.get('.context-menu .menu-item').contains('Create Category').click();
    cy.get('.context-menu input[placeholder="Enter category name"]').should('be.visible');
    cy.get('.context-menu input[placeholder="Enter description"]').should('be.visible');
  });
});
